...
async function loadInitialState() {
    await Promise.all([loadExamples(), loadCurrentUser()]);
}
...
async function loadCurrentUser() {
    try {
        ...
        console.warn("No se pudo verificar el usuario actual:", error);
    }
}
...
function renderExampleButtons() {
    ...
    if (examples.length === 0) {
        examplesDrawer.innerHTML = '<div class="history-empty">Todavia no hay ejemplos disponibles.</div>';
        return;
    }
    ...
    examplesDrawer.innerHTML = `
        <div class="drawer-grid">
            ${examples
                .map(
                    (example, index) => `
                        <button class="example-card" data-example-index="${index}" type="button">
    ...
    examplesDrawer.querySelectorAll(".example-card").forEach((button) => {
